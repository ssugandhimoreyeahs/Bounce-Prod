import Dollar from './DollarSvg.svg'
import AddBlueSmall from './AddBlueSmall.svg'
import ContactWhite from './ContactWhite.svg'
import List from './List.svg'
import Search from './Search.svg'
import ToggleGrey from './ToggleGrey.svg'
import Info from './Info.svg'
import BlackCircleCross from './BlackCircleCross.svg'
import BounceLogo from './BounceLogo.svg'
import InfoWhite from './InfoWhite.svg'
import LeftBlueArrow from './LeftBlueArrow.svg'
import RightBlueArrow from './RightBlueArrow.svg'
import DollarWhite from './DollarWhite.svg'
import Message from './Message.svg'
import Delete from './Delete.svg'
import HirePeople from './HirePeople.svg'
import Peoples from './Peoples.svg'
import TagPrice from './TagPrice.svg'
import PlusWhiteCalender from './PlusWhiteCalender.svg'
import DirectionBlue from './DirectionBlue.svg'
import Download from './Download.svg'
import Chat from './Chat.svg'
import BlueArrowDown from './BlueArrowDown.svg'
import PenWhite from './PenWhite.svg'
import Google from './Google.svg'
import Insta from './Insta.svg'
import Apple from './Apple.svg'
import StarWhite from './StarWhite.svg'
import DollarOnlyWhite from './DollarOnlyWhite.svg'
import WebsiteBlack from './WebsiteBlack.svg'
import AddBlueWhite from './AddBlueWhite.svg'
import MessageBlack from './MessageBlack.svg'
import FavouritedHeart from './FavouritedHeart.svg'
import FavouriteHeart from './FavouriteHeart.svg'
import BlackOutlineShare from './BlackOutlineShare.svg'
import UploadBlue from './UploadBlue.svg'
import GreyBell from './GreyBell.svg'
import GreyParty from './GreyParty.svg'
import GreyPerson from './GreyPerson.svg'
import DJ from './DJ.svg'
import PhotoVideo from './PhotoVideo.svg'
import Certified from './Certified.svg'
import Armed from './Armed.svg'
import Multilingual from './Multilingual.svg'
import BlackMenubar from './BlackMenubar.svg'
import BlackClose from './BlackClose.svg'
import BlackBorderPen from './BlackBorderPen.svg'
import More from './More.svg'
import WhiteCalender from './WhiteCalender.svg'
import Scanner from './Scanner.svg';
import BlackBell from './BlackBell.svg'
import BlackParty from './BlackParty.svg'
import BlackPerson from './BlackPerson.svg';
import BlueCamera from './BlueCamera.svg'
import BlackContact from './BlackContact.svg'
import InfoBlackBorder from './InfoBlackBorder.svg'
import Equipments from './Equipments.svg'
import Cuisines from './Cuisines.svg'
import Services from './Services.svg'
import BlackArrowDown from './BlackArrowDown.svg'
import Saved from './Saved.svg'
import AppleMusic from './AppleMusic.svg'
import Spotify from './Spotify.svg'
import Twitter from './Twitter.svg'
import Snapchat from './Snapchat.svg'
import Tiktok from './Tiktok.svg'
import LockBlack from './LockBlack.svg'
import Settings from './Settings.svg'
import BlackFilter from './BlackFilter.svg'
import RequestSent from './RequestSent.svg'
import SendRequest from './SendRequest.svg'
import BlackOutlineHeart from './BlackOutlineHeart.svg'
import WhiteMessageOutline from './WhiteMessageOutline.svg'
import GreyHamburger from './GreyHamburger.svg'
import Bartender from './Bartender.svg'
import DjMusic from './DJ.svg'
import Food from './Food.svg'
import Security from './Security.svg'
import Cleaning from './Cleaning.svg'
import PartyRentals from './Party_Rentals.svg'
import Videographer from './Video.svg'
import Bounce from './Bounce_Logo.svg'
import Pdf from './Pdf.svg'
import BounceProLogo from './BounceProLogo.svg'
import BounceSplash from './Bounce.svg'
import BlueCross from './BlueCross.svg'
import BlackDollar from './BlackDollar.svg'
import GreyEye from './GreyEye.svg'
import BlueEye from './BlueEye.svg'
import Webpin from './Webpin.svg'
import AddBlue from './AddBlue.svg'
import CohostWhite from './CohostWhite.svg'
import BlackPen from './BlackPen.svg'
import GreenTick from './GreenTick.svg'
import WhiteTick from './WhiteTick.svg'
import WhiteShare from '../white_svg/Share.svg';
import WhiteMore from '../white_svg/More_Icons.svg';
import EditPen from './EditPen.svg';
import YellowStar from './YellowStar.svg';
import GreyHome from './GreyHome.svg';
import GreyEvent from './GreyEvent.svg';
import BlackEvent from './BlackEvent.svg';
import BlackHome from './BlackHome.svg';
import WhitePerson from './WhitePerson.svg'
import GreyCross from './GreyCross.svg'
import InstaNew from './InstaNew.svg'
import Add_Outline from './Add_Outline.svg'
import Bell_Outline from './Bell_Outline.svg'
import Home_Outline from './Home_Outline.svg'
import Search_Outline from './Search_Outline.svg'
import Add_Fill from './Add_Fill.svg'
import Bell_Fill from './Bell_Fill.svg'
import Home_Fill from './Home_Fill.svg'
import Search_Fill from './Search_Fill.svg'
import ThreeBlackDots from './ThreeBlackDots.svg'
import BlackOutlineCalender from './BlackOutlineCalender.svg'
import BlackSolidStar from './BlackSolidStar.svg'
import BlackSolidShare from './BlackSolidShare.svg'
import BlackSolidPeoples from './BlackSolidPeoples.svg'

import Going from './Going.svg'
import Interested from './Interested.svg'
import Cantgo from './Cantgo.svg'
import Linkedin from './Linkedin.svg'
import Party_Outline from './Party_Outline.svg'
import FavouriteMusic from './FavouriteMusic.svg'
import ChangeBlue from './ChangeBlue.svg'
import BlueTick from './BlueTick.svg'
import CohostPurple from './CohostPurple.svg'
import Account_Outline from './Account_Outline.svg'



export {

    Account_Outline,
    CohostPurple,
    BlueTick,
    ChangeBlue,
    FavouriteMusic,
    Party_Outline,
    Linkedin,
    Going,
    Interested,
    Cantgo,
    BlackSolidPeoples,
    BlackSolidShare,
    BlackSolidStar,
    BlackOutlineCalender,
    ThreeBlackDots,
    Add_Fill,
    Bell_Fill,
    Home_Fill,
    Search_Fill,
    Add_Outline,
    Bell_Outline,
    Home_Outline,
    Search_Outline,
    InstaNew,
    GreyCross,
    WhitePerson,
    BlackEvent,
    BlackHome,
    GreyEvent,
    GreyHome,
    YellowStar,
    EditPen,
    WhiteTick,
    GreenTick,
    BlackPen,
    CohostWhite,
    AddBlue,
    Webpin,
    GreyEye,
    BlueEye,
    BlackDollar,
    BlueCross,
    BounceSplash,
    BounceProLogo,
    Pdf,
    Bounce,
    Food,
    Bartender,
    DjMusic,
    Cleaning,
    PartyRentals,
    Videographer,
    GreyHamburger,
    WhiteMessageOutline,
    BlackOutlineHeart,
    RequestSent,
    SendRequest,
    BlackFilter,
    Settings,
    LockBlack,
    Twitter,
    Snapchat,
    Tiktok,
    AppleMusic,
    Spotify,
    Saved,
    BlackArrowDown,
    Services,
    Cuisines,
    Equipments,
    InfoBlackBorder,
    BlackContact,
    BlueCamera,
    BlackBell,
    BlackPerson,
    BlackParty,
    Scanner,
    WhiteCalender,
    BlackBorderPen,
    More,
    BlackClose,
    BlackMenubar,
    Certified,
    Armed,
    Multilingual,
    DJ,
    PhotoVideo,
    Security,
    GreyBell,
    GreyParty,
    GreyPerson,
    UploadBlue,
    BlackOutlineShare,
    FavouriteHeart,
    FavouritedHeart,
    MessageBlack,
    AddBlueWhite,
    WebsiteBlack,
    DollarOnlyWhite,
    StarWhite,
    Apple,
    Insta,
    Google,
    PenWhite,
    BlueArrowDown,
    Download,
    Chat,
    DirectionBlue,
    TagPrice,
    PlusWhiteCalender,
    Peoples,
    HirePeople,
    Message,
    Delete,
    DollarWhite,
    LeftBlueArrow,
    RightBlueArrow,
    InfoWhite,
    BounceLogo,
    BlackCircleCross,
    Info,
    Dollar,
    AddBlueSmall,
    ContactWhite,
    List,
    Search,
    ToggleGrey,
    WhiteShare,
    WhiteMore
}